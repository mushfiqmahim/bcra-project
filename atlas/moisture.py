"""Sentinel-2 NDMI monthly time series for Bangladesh districts."""
...
def _add_ndmi(image: ee.Image) -> ee.Image:
...
def ndmi_timeseries(
    ...
    """Compute monthly mean NDMI for a Bangladesh district.

    Builds a single Sentinel-2 image with one NDMI band per month over the
    ...
    Args:
        ...
        end_date: Right edge of the window, ISO 'YYYY-MM-DD'.

    Returns:
        DataFrame with columns 'date' (datetime64, first-of-month) and
        'ndmi' (float, NaN for empty months), sorted by date ascending.
    """
    end_py = pd.Timestamp(end_date).normalize()
    ...
    for i in range(months):
        ...
        band_name = f"ndmi_{m_start_py.year:04d}_{m_start_py.month:02d}"
        band_names.append(band_name)
        dates.append(
            pd.Timestamp(year=m_start_py.year, month=m_start_py.month, day=1)
        )

        monthly = collection.filterDate(m_start_ee, m_end_ee).select("NDMI")
        band = ee.Image(
            ee.Algorithms.If(
                monthly.size().gt(0),
    ...
    df = pd.DataFrame(rows, columns=["date", "ndmi"])
